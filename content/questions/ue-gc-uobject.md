...
GC в UE работает через mark-and-sweep: есть корневые объекты (мир,引擎, активоры), из них рекурсивно помечаются все достижимые UObject. Непомеченные удаляются. Сборка запускается примерно раз в 60 секунд или по требованию. Важно понимать разницу между UObject* (сильная), TWeakObjectPtr (слабая, обнуляется) и TSoftObjectPtr (ленивая загрузка). Для кастомных указателей нужно переопределять AddReferencedObjects.
