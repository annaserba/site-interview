...
Kafka Streams: обработка событий из Kafka topics. Windowing: сумма заказов за последние 5 минут (tumbling window). Exactly-once: транзакции между read и write. Backpressure:consumer lag растёт — broker apply. Flink: stateful processing, checkpointing для fault tolerance. Сравнение: Spark Streaming — micro-batch (latency ~100ms), Flink — true streaming (latency ~1ms).
