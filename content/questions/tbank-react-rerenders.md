...
Ререндер: state, контекст, рендер родителя, внешний store. Render ≠ DOM commit. Оптимизируйте после профилирования: локализуйте state, мемоизируйте дорогие границы.

## Что ожидают в ответе

Проверяются модель render/commit и умение профилировать, а не перечисление memo-хаков. Ожидается, что кандидат различает триггеры ререндера и DOM commit, демонстрирует использование Profiler/DevTools и понимает, что `useMemo`/`useCallback` стоят памяти и не должны применяться механически.
