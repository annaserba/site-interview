...
Frontend (localhost:3000) → API (api.example.com). Без CORS: blocked. С CORS: `Access-Control-Allow-Origin: http://localhost:3000`. Preflight: OPTIONS → сервер отвечает哪些 methods allowed → actual request.
