...
sourceUrl: ""
sourceVideos: [{"company":"Frontend-интервью","url":"https://www.youtube.com/watch?v=a43a-SCCHLg&t=2593s","title":"Event Loop: макрозадачи и микрозадачи"}]
---
...
- Утверждать, что requestAnimationFrame выполняется в той же очереди, что и setTimeout, игнорируя этап рендеринга.
- Путать requestIdleCallback с requestAnimationFrame: idle callback выполняется только после отрисовки кадра в свободное время.
- Не упоминать, что setTimeout(0) имеет минимальную задержку ~4 мс, в то время как MessageChannel срабатывает без неё.
- Проектировать бесконечную цепочку микротаск, не осознавая риска starving рендеринга UI.
- Использовать scheduler.yield() без понимания, что он отдаёт контроль браузеру для рендеринга, а не просто переносит задачу.
